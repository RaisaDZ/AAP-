...
num_experts <- 12
formula <- "Price2~Property1+Property2+Property3+flag_leasehold+  
                  flag_newbuilding+dist1+dist21+Station1+Station2+
                  Income_score+Employment_score+HD_score+Crime_score+
                  LIV_score+Indoors_score+Outdoors_score+
                  GB_score+WB_score+Child_score+Skills_score+    
...
for (i in 1:num_experts)  {
  ...
  df[[paste("pred", i, sep="")]] <- predict(lmfit, df)
...
#export to csv
write.csv(losses, "losses_london1.csv", row.names = FALSE)
